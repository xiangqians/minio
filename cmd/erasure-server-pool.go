...
func (z *erasureServerPools) putObjectWithThumb(idx int, ctx context.Context, bucket string, object string, data *PutObjReader, opts ObjectOptions) (ObjectInfo, error) {
	// 创建分流缓存器
	var buf = newThumbTeeBuffer(bucket, object, data, opts)
	if buf != nil {
		data.Tee(buf)
	}
	...
	// 处理是否成功
	if buf.Ok() {
		tag, err := genThumb(context.Background(), bucket, object, buf, buf.Written(), buf.Readable(), buf.Duration())
		if err != nil {
			thumb.Warn("putObjectWithThumb-%s bucket=%s, object=%s, %v", tag, bucket, object, err)
		}
	}
